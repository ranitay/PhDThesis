...
The high-energy channel analysis employs a binned likelihood function. Observed and expected event yields are compared in the nine ROI log$(\cSiib/\cSi),\cSi{})$-bins described in Sec.~\ref{subsubsec:HighE}. 
...
\end{minipage}
\mycaption[The \Xehund\ limits (90\%\,C.L.$_S$) on isoscalar dimensionless coupling for all elastic scattering EFT operators]{The \Xehund\ limits (90\%\,C.L.$_S$) on isoscalar dimensionless coupling for all elastic scattering EFT operators. The limits are indicated in solid black lines. The expected sensitivity is shown in green and yellow(1$\sigma$ and 2$\sigma$ respectively). Limits from CDMS-II Si, CDMS-II Ge, and SuperCDMS \cite{CDMSEFT} are presented as blue asterisks, green triangles, and orange rectangles, respectively. For operators 3 and 8, a full limit was published, for all other operators only $m_\chi = 10$ and $m_\chi =300$ are available.}
